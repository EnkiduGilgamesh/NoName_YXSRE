/*
 * File: \extension\英雄杀RE\help\example\yxs.js
 * Project: noname
 * Created Date: Friday Aug 12th 2022, 3:31:30 pm
 * Author: Wenren Muyan
 * Comments: 
 * --------------------------------------------------------------------------------
...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
		...
		character:{
			/*yxs_qinqiong:["male","qun",4,["yxs_fanji","yxs_menshen"],[]],
			yxs_wuzetian:['female','shen',4,['nvquan','qiandu','yxsweiyi']],
			yxs_caocao:['male','wei',4,['zhulu','xieling']],
			yxs_mozi:['male','qun',3,['jieyong','feigong','jianai']],
			yxs_bole:['male','qun',3,['bolehuiyan','xiangma']],
			yxs_aijiyanhou:['female','western',3,['seyou','sheshi']],				//DONE: 
			yxs_diaochan:['female','qun',3,['yxs_zhunwu','biyue']],					//DONE
			yxs_yangyuhuan:['female','qun',3,['fengyan','nichang']],
			yxs_baosi:['female','qun',3,['jieyin','fenghuo']],
			yxs_napolun:['male','western',4,['tongling','fanpu']],
			yxs_kaisa:['male','western',4,['ducai']],
			yxs_zhuyuanzhang:['male','shen',4,['qiangyun']],						//DONE
			// yxs_jinke:['male','qun',3,['cike','qiangxi']],
			yxs_libai:['male','shen',3,['miaobi','zhexian']],
			yxs_luban:['male','qun',3,['guifu','lshengong']],						//DONE
			...
			yxs_yujix:['female','qun',3,['yxs_chuyao','yxs_bieji']],				//DONE: 
			yxs_xiangyu:['male','chu',4,['yxs_wushuang_modi','yxs_bawang']],		//DONE:
			yxs_yingzheng:['male','qin',4,['yxs_batu','yxs_yuyu'],['zhu']],			//DONE:
			yxs_yuefei:['male','qun',4,['wumu']],									//DONE
			yxs_fuermosi:['male','western',3,['yanyi','jiean']],
			...
			yxs_meixi:['female','qun',3,['liebo','yaoji']],
			yxs_lanlinwang:['male','qun',4,['guimian','lyuxue']],*/
		},
		characterIntro:{
   ...
   ai:{
     ...
     forced:true,
          ...
          var target=player.storage.yxs_menshen2;
			...
			yxs_taiji:{
				...
				content:function(){
					...
					var num_red=player.countCards('h',{color:'red'});
					var num_black=player.countCards('h',{color:'black'});
					if(num_red==num_black) player.draw();
				},
				ai:{
					order:10,
					result:{
			...
			wumu:{
				...
				group:['wumu_pass','wumu_black','wumu_red'],
				subSkill:{
					pass:{
						mod:{
			...
			yxs_chuyao:{
				...
				content:function(){
					'step 0'
					player.chooseTarget(get.prompt('yxs_chuyao'),'交给一名其他角色任意张牌',function(card,player,target){
						return target!=player;
					}).set('ai',function(target){
						return get.attitude(player,target);
					});
					...
					player.getHistory('lose',function(evt){
						if(evt.getParent(2).name=='yxs_chuyao') num+=evt.cards.length;
					});*/
			...
			yxs_bieji:{
				...
				skillAnimation:true,
				animationColor:"fire",
				derivation:'yxs_wushuang_modi',
				content:function(){
					"step 0"
					player.chooseTarget(get.prompt('yxs_bieji'),function(card,player,target){
					...
					if(result.bool){
						...
						if(target.hasSkill('wuhsuang')){
							...
							target.addSkill('yxs_wushuang_modi');
						}
			...
			yxs_wushuang_modi:{				//TODO: AI rewrite
				shaRelated:true,
				//audio:2,
				//audioname:["re_lvbu","shen_lvbu","lvlingqi"],
				forced:true,
				locked:true,
				group:["yxs_wushuang_modi_1","yxs_wushuang_modi_2"],
				preHidden:["yxs_wushuang_modi_1","yxs_wushuang_modi_2"],
				subSkill:{
					1:{
						trigger:{
						...
						content:function(){
							...
							player.chooseControl(list).set('prompt',get.prompt('yxs_wushuang_modi_1')).set('choiceList',choiceList).set('ai',function(){
								var player=_status.event.player;
			...
			yxs_bawang:{
				...
				forced:true,
				filter:function(event,player){
					return (player.identity=='fan'||player.identity=='nei')&&event.player.identity=='zhu'&&player.getEnemies().length>1;
				},
						...
						content:function(){
							...
							if(num){
								if(!trigger.player.countCards('he'))
									...
									if(trigger.player.countCards('he')<num) num=trigger.player.countCards('he');
									trigger.player.chooseBool('是否交给项羽'+get.cnNumber(num)+'张牌？').set('ai',function(){
										if(num<=1) return true;
										if(num>3) return false;
										if(trigger.player.hp==1) return true;
										if(trigger.player.countCards('he')>=num+2) return true; //TODO: 
										return false;
									});
									if(result.bool){
										trigger.player.chooseCard(num, 'he', '选择'+get.cnNumber(num)+'张牌交给项羽', function (card) {
											return true;
										}).set('ai', function (card) {
											return 3 - get.value(card);
										});
			...
			yxs_juecha:{		//TODO: ai rewrite
				trigger:{player:'damageEnd'},
				priority:15,
				content:function(){
					'step 0'
					var storage1=player.getStorage('yxs_juecha_round');
					...
					list.push('cancel2');
					player.chooseControl(list).set('prompt',get.prompt('yxs_juecha')).set('choiceList',choiceList).set('ai',function(){
						var player=_status.event.player;
						var list=_status.event.controls.slice(0);
						var gett=function(choice){
					...
					'step 2'
					player.logSkill('yxs_juecha');
					player.addTempSkill('yxs_juecha_round','roundStart');
					player.markAuto('yxs_juecha_round',[event.index]);
					switch(event.index){
						case 0:
							player.addTempSkill('yxs_juecha_sha','roundStart');
							break;
						case 1:
							player.addTempSkill('yxs_juecha_trick','roundStart');
							break;
						case 2:
			...
			tiewan:{
				...
				content:function(){
					'step 0'
					player.choosePlayerCard(player,'he',get.prompt('tiewan',trigger.player)).set('ai',function(card){
			...
			qianglue:{
				...
				priority:-1,
				filter:function(event){
					return event.target.countCards('he')>0&&get.color(event.card)=='black';
				},
				check:function(event,player){
				...
				content:function(){
					...
					if(player.countCards('he')){
						player.chooseCardTarget({
							...
							prompt2:"弃置一张装备牌然后获得一名男性角色的所有技能直到首次受到伤害",
			...
			tongling:{
				...
				filter:function(event,player){
					...
					return event.player&&event.player.isFriendsOf(player)&&(type=='basic'||type=='trick')&&get.tag(event.card,'damage')>0&&player.storage.tongling<5;
				},
				content:function(){
			...
			qiangyun:{
				...
				content:function(){
					'step 0'
					var num1=player.countMark('qiangyun');
					var num2=2*(num1+1);
					event.num=num2;
					...
					player.addMark('qiangyun',1,false);
				
					'step 2'
					if(player.countCards('h')>2*game.players.length){
						player.removeMark('qiangyun',player.countMark('qiangyun'),false);
					}
				},
				ai:{
					threaten:0.8,
			...
			miaobi:{
				...
				//filterTarget:function(card,player,target){
				//	if(target==player) return false;
				//	var stat=player.getStat('skill').miaobi_targets;
				//	return !stat||!stat.contains(target);
				//},
				
				filterTarget:function(card,player,target){
					return target!=player&&target.countCards('he')>0//&&game.hasPlayer((current)=>lib.skill.miaobi.filterTarget(null,player,current));;
				},
				content:function(){
					'step 0'
    				...
    				//if(!stat.miaobi_targets) stat.miaobi_targets=[];
        			//stat.miaobi_targets.push(target);
				},
				ai:{
					order:9,
					result:{
		...
		translate:{
			...
			yxs_fanji:"反击",
            yxs_fanji2:"反击",
            yxs_fanji_info:"当你受到【杀】或【决斗】造成的伤害后，你可以对伤害来源使用一张【杀】。若此【杀】为红色，则其不可闪避",
            yxs_menshen:"门神",
            yxs_menshen2:"门神",
            yxs_menshen3:"门神",
            yxs_menshen_info:"结束阶段，你可以选择一名其他角色，若如此做，直到你的下回合开始，对该角色使用的【杀】或【决斗】将目标转移给你",
			zhuxin:'诛心',
			...
			guimian:'鬼面',
			guimian_info:'锁定技，当你在出牌阶段使用【杀】造成伤害后，本阶段内你可以多使用一次【杀】',
			lyuxue:'浴血',
			lyuxue2:'浴血',
			lyuxue_info:'锁定技，当你对一名角色造成一次伤害后，其获得一个“浴血”标记；当一名角色失去浴血标记后，其流失一点体力；准备阶段，若场上“浴血”标记的数量不少于存活角色数的一半（向下取整），然后移除所有“浴血”标记；当你死亡后，移除所有“浴血”标记',
			huli:'护理',
			...
			yxs_wudang:'武当',
			yxs_wudang_info: '当你受到伤害后，你可以展示手牌，若手牌均为黑色，你回复1点体力；若均为红色，你对伤害来源造成1点伤害；若你没有手牌，则你可以依次执行前面两个效果',
			yxs_taiji:'太极',
			yxs_taiji_info:'出牌阶段，若你手牌中红色牌数等于黑色牌数，你可以摸一张牌。',
			luobi:'落笔',
			...
			zyhufu:'胡服',
			zyhufu_info:'锁定技，若你的装备区内没有防具牌时，摸牌阶段你多摸一张牌；当你装备区内有防具牌时，你的手牌上限+3',
			hanbei:'捍北',
			hanbei_info:'锁定技，若你的装备区有坐骑牌时，你的【杀】不可被【闪】抵消',
			kuangchan:'狂禅',
			kuangchan_info:'锁定技，你做主公时，不增加体力上限',
			yxs_dili:'底力',
			// dili_info:'锁定技，摸牌阶段，你额外摸X张牌，X为你已损失的体力值',
			yxs_dili_info:'锁定技，摸牌阶段，你多摸X+1张牌，X为你已损失的体力值',
			chujia:'初嫁',
			...
			wumu:'武穆',
			wumu_info:'锁定技，你的黑色【杀】无视距离，且伤害+1；红色【杀】不计入回合内的出杀限制，且不可闪避',
			ysheshen:'舍身',
			ysheshen_info:'每当你受到一点伤害，可以观看牌堆顶的两张牌，并将其交给任意1~2名角色',
			yxs_chuyao:'楚腰',
			yxs_chuyao_info:'当你受到伤害后，你可以将任意张牌交给一名其他角色，然后若你交出的牌不小于：一张，你摸两张牌；两张，你对伤害来源造成一点伤害；三张，你增加1点体力上限；四张，你回复1点体力',
			yxs_bieji:'别姬',
			yxs_bieji_info:'当你进入濒死状态时，或出牌阶段结束时，你可以将区域内所有牌交给一名男性角色，若你的体力上限大于其，其增加体力上限至你的体力上限和等量体力，然后获得“无双”，若其已拥有“无双”，则修改“无双”，最后你死亡。',
			yxs_wushuang_modi:'无双',
			yxs_wushaung_modi_info:'锁定技。当你使用【杀】/【决斗】指定目标后（或成为【决斗】的目标后），你须选择一项：1，摸两张牌，然后此牌无效；2，摸一张牌；3，令此牌需要响应的【闪】/【杀】数量+1，4，弃置一张牌，令此牌需要响应的【闪】/【杀】数量+2。',
			yxs_bawang:'霸王',
			yxs_bawang_info:'反贼技。锁定技。主公的出牌阶段开始时，其需选择一项，1，交给你牌使你的手牌数与体力上限相等（不足则全给，不需要则不给）；2，受到1点伤害。当你杀死主公时，你与其交换身份，然后你增加1点体力上限和体力，最后将手牌摸至体力上限。',
			yxs_batu:'霸图',
			yxs_batu_info:'一名角色的回合结束时，你可以将手牌数补至X，X为场上的势力数',
			yxs_yuyu:'御宇',
			yxs_yuyu_info:'主公技。当你杀死一名非忠臣角色时，你可以减一点体力上限，将其身份改为忠臣，势力改为“秦”，然后其失去所有技能，将体力上限改为4，体力值回复至2，弃置所有牌，最后摸两张牌。',
			sanbanfu:'三板斧',
			sanbanfu_info:'当你对其他角色使用杀时，你可以使此杀有如下效果：若对方没有出闪，其受到2点伤害；若对方打出了一张闪，你与其各受到1点伤害；若对方打出了两张闪，你受到一点伤害',
			bingsheng:'兵圣',
			bingsheng_info:'出牌阶段限一次，你可以弃置X张花色不同的手牌，指定一名其他角色使其体力值与你相同（体力最多变化X点）',
			taolue:'韬略',
			taolue_info:'锁定技，你的手牌上限+13',
			shentan:'神探',
			shentan_info:'出牌阶段限一次，你可以弃置一张牌，获得距离2以内的一名角色的一张手牌，然后可以将其交给另一名角色',
			hanqiang:'寒枪',
			hanqiang_info:'锁定技，你没装备武器时，攻击范围为3',
			biaoqi:'骠骑',
			biaoqi_info:'锁定技，当你使用【杀】指定目标后，若你的攻击范围大于目标体力值，则此【杀】不可闪避；若你的攻击范围小于目标体力值，你摸一张牌',
			wluoyan:'落雁',
			...
			yxs_juecha:'觉察',
			yxs_juecha_info:'‌当你受到伤害后，你可以选择本轮次未选择的一项直到下一轮开始，1，【杀】对你无效；2，通常锦囊牌对你无效；3，摸X张牌，X为你本轮已选择的项数。',
			yxs_juecha_sha:'觉察',
			yxs_juecha_trick:'觉察',
			tiewan:'铁腕',
			tiewan_info:'当你受到伤害后或有其他角色使用延时锦囊牌后，你可以将一张红色牌当作【乐不思蜀】或将一张黑色牌当作【兵粮寸断】使用',
			qianglue:'强掠',
			qianglue_info:'当你的黑色【杀】被闪避后，你可以获得对方的一张牌',
			xiadao:'侠盗',
			...
			yizhuang:'易装',
			yizhuang2:'易装',
			yizhuang_info:'准备阶段，你可以弃置一张装备牌并选择一名男性角色，获得其所有技能，直到你首次受到伤害',
			kongju:'控局',
			kongju_info:'锁定技，你的手牌上限为你的体力上限；当你的手牌数小于体力上限时，你不能成为过河拆桥或顺手牵羊的目标；当你的手牌数大于体力上限时，你不能成为乐不思蜀的目标',
			tuqiang:'图强',
			tuqiang_info:'当你使用或打出一张点数不小于10的牌时，你可以摸两张牌。',
			zhensha:'鸩杀',
			zhensha_info:'当有角色进入濒死状态时，你可以弃置一张【酒】或两张黑色牌，然后该角色将体力回复至1，然后受到X+1点伤害，X为致其进入濒死状态的伤害数或体力流失数。',
			xumou:'蓄谋',
			xumou_info:'结束阶段，你可以摸X张牌，X为场上体力值等于体力上限的角色数，然后若X不小于3，你翻面。',
			guifu:'鬼斧',
			...
			zhexian:'谪仙',
			zhexian_info:'当你于一名其他角色的回合内失去牌时，你可以摸一张牌',
			miaobi:'妙笔',
			miaobi_info:'出牌阶段限四次，你可以弃置一名其他角色的一张牌，若此牌是基本牌或普通锦囊，你可以将一张手牌当此牌使用；否则你须弃置一张牌',
			cike:'刺客',
			cike_info:'你对别的角色出【杀】时可以选择做一次判定：若判定牌为红色花色，则此【杀】不可回避，直接命中；若判定牌为黑色花色，你可以选择弃掉对方一张牌。',
			qiangyun:'强运',
			qiangyun_info:'当你失去最后一张手牌，可摸2*(X+1)张牌，X为你已发动此技能的次数。然后若你的手牌数大于存活角色数的两倍，你将X归零',
			ducai:'独裁',
			ducai2:'独裁',
			ducai3:'独裁',
			ducai_info:'出牌阶段限一次，你可以弃置一张牌，则本轮内除你外的角色不能使用或打出与该手牌花色相同的手牌',
			tongling:'统领',
			tongling_info:'锁定技，当一名友方角色使用一张带伤害标签的牌后，你获得1个“统领”标记（标记上限为5）',
			fanpu:'反扑',
			fanpu_info:'出牌阶段限一次，你可以移去3枚“统领”标记，然后视为对至多3名角色使用一张【杀】(无距离限制)',
			fenghuo:'烽火',
			...
			yxs_zhunwu:'衠舞',
			yxs_zhunwu_info:'出牌阶段开始时，你可以展示所有手牌，若其中的牌的类型数不小于：1，你可以选择一名男性角色，你与其各摸一张牌；2，你获得“离间”直到此阶段结束；3，你获得“离魂”直到此阶段结束。',
			nichang:'霓裳',
			nichang2:'霓裳',
